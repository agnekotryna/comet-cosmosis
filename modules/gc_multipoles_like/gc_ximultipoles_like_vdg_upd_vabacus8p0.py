...
5/8/2025 4.4.4 version without h scaling just for fun -- I think comet does this internally actually
5/8/2025 4.4.5 added avir conversion to Mpc units for EFT_vdg model 
25/8/2025 5.0 added the option of using non-Alex bias (copied from 4.1 but with all the updates now) + option to use this with "true" input value of As (in e-9 units)
3/9/25 5.3 assume all units are given in Mpc but the fitting is still done in Mpc/h. We leave avir as is (no internal conversion for this and we trained in Mpc), we convert counterterms
10/9/25 7.0 copied fro 5.3 (not 6), to be used with new neutrino comet
10/9/25 7.1 copied fro 5.3 (not 6), to be used with new neutrino comet - try to make neutrinos work
17/3/26 8.0 projection effect sampling
"""
...
def compute_reparam_factors(emu, params_i, sigma12_fid, D_fid, As_fid, de_model):
	"""
	Compute AP amplitude and approximate sigma12 for reparameterization.
	AP factor: A_AP = 1 / (q_tr^2 * q_lo)
	sigma12 approx: sigma12_fid * D(z)/D_fid(z) * sqrt(As/As_fid); 
	approximation ignoring transfer function variation 
	(adequate for this purpose; the reparameterization itself is a first-order approximation) 
	"""
	Mnu = params_i.get('Mnu', 0.0)
	wnu = Mnu / emu.neutrino_mass_fac
	Om0 = (params_i['wc'] + params_i['wb'] + wnu) / params_i['h']**2
	H0 = 100.0 * params_i['h']
	z = np.atleast_1d(params_i['z'])
	w0_val = params_i.get('w0', -1.0)
	wa_val = params_i.get('wa', 0.0)

	emu.cosmo.update_cosmology(Om0, H0, de_model=de_model, w0=w0_val, wa=wa_val)

	# AP factor
	q_lo = emu.H_fid / emu.cosmo.Hz(z)
	q_tr = emu.cosmo.comoving_transverse_distance(z) / emu.Dm_fid
	if not emu.use_Mpc:
		q_lo *= params_i['h']
		q_tr *= params_i['h']
	APfac = 1.0 / (float(np.squeeze(q_tr))**2 * float(np.squeeze(q_lo)))

	# Approximate sigma12
	D_z = float(np.squeeze(emu.cosmo.growth_factor(z)))
	sigma12 = sigma12_fid * (D_z / D_fid) * np.sqrt(params_i['As'] / As_fid)

	return APfac, sigma12
...
def load_cosmology(block, de_model, lenz, mnu=False, trueAs=False):
	...
	trueAs: if True, expect as input amplitude As in 1e-9, so need to scale for comet
	"""
	cosmo = {}
	...
	try:
		...
		if trueAs == False: cosmo["As"] = np.repeat(block.get_double(names.cosmological_parameters, "a_s"), lenz)
		elif trueAs == True: cosmo["As"] = np.repeat(block.get_double(names.cosmological_parameters, "a_s"), lenz)*1e9
	...
	if mnu:
		try:
			cosmo["Mnu"] = np.repeat(block.get_double(names.cosmological_parameters, "mnu"), lenz)
		except:
			print("Error reading input Mnu")
	#	cosmo["Mnu"] = np.repeat(block.get_double(names.cosmological_parameters, "mnu"), lenz)
	#except:
	#	pass
...
def trim_scales(cov, nmin, nmax, nfull, nmeas):
	"""
	...
	nmax : int
		the maximum point USED (the biggest scale         
	nfull : int 
		the number of points in the full measurement without cuts
	nmeas : int
	...
	# Check if covariance matches expected size
	expected_len = nmeas * nfull
	if cov_len != expected_len:
		print(f"WARNING: Covariance length ({cov_len}) does not match nmeas*len_meas ({expected_len}).")
		# Assume covariance is for 3 measurements, but only 2 are used
		if cov_len == 3 * nfull:
			print("Assuming covariance is for 3 measurements, but only 2 are used. Cutting out the third measurement.")
			# Indices for the third measurement
			third_meas_start = 2 * nfull
			third_meas_end = 3 * nfull
			bins_to_delete = list(range(third_meas_start, third_meas_end))
			cov = np.delete(cov, bins_to_delete, axis=0)
			cov = np.delete(cov, bins_to_delete, axis=1)
			cov_len = cov.shape[0]
			print(f"Covariance shape after cutting third measurement: {cov.shape}")
		else:
			print("Covariance size does not match any expected pattern. Check your inputs!")
	...
	print(f"Covariance shape after cutting: {cov_trim.shape}")
...
def setup(options):
	...
	if (RSD_model == 'pk_vdg') or (RSD_model == 'pk_vdg_nonu'):
		print ('Using Fourier COMET model')
		kcut_val = options.get_double(option_section, "kcut_val", default=2.5)
		print ('kcut_val =', kcut_val)
		print ()
	...
	if bias_basis == 'EggScoSmi':
		...
		bG2CoEvol = options.get_bool(option_section, "bG2CoEvol")
		print ('Using bG2 relation:', bG2CoEvol)
		print ()
	
		bGam3CoEvol = options.get_bool(option_section, "bGam3CoEvol")
		print ('Using bGam3 relation:', bGam3CoEvol)
		print ()

		g21CoEvol = False
		g2ExSet = False
	else: 
		raise ValueError("Bias basis not recognised, please choose between 'EggScoSmi' and 'AssBauGre'")

	trueAs = options.get_bool(option_section, "trueAs", default=False)
	print ('Using trueAs scaling:', trueAs)
	print ()

	use_AP_reparam = options.get_bool(option_section, "use_AP_reparam", default=False)
	print ('Using AP+sigma12 reparameterization:', use_AP_reparam)
	print ()

	scale_avir_by_f = options.get_bool(option_section, "scale_avir_by_f", default=False)
	print ('Sampling f*avir (scale_avir_by_f):', scale_avir_by_f)
	print ()
	...
	for i,file in enumerate(data_files.split(" ")):
		...
		cov_cut, num_points_use = trim_scales(cov, min_points_use, max_points_use, num_points_full,n_meas)
		if n_mocks == 0: cor_fact = 0.0
		else: cor_fact = (n_meas * num_points_use + 1)/(n_mocks - 1) #Hartlap correction, assuming 3 measurements
		...
		assert n_meas * data_cut.shape[0] == cov_cut.shape[0], "Data and covariance row mismatch"
		assert data_cut.shape[0] == windows_cut.shape[0], "Data and window row mismatch"
		assert rbands_cut.shape[0] == windows_cut.shape[1], "Bands and window column mismatch"
		print ("Data, covariance, bands and windows loaded successfully")
		...
		if RSD_model in ['EFT_vdg_EH', 'EFT_vdg']:
			emu.change_bias_basis(bias_basis) #AS
		else:
			emu.change_basis(bias_basis) #AS
		print ("Using bias basis "+bias_basis) #AS
		print()

		if use_AP_reparam:
			# Store fiducial growth factor and As for sigma12 approximation
			D_fid_val = float(np.squeeze(emu.cosmo.growth_factor(np.atleast_1d(redshift[i]))))
			As_fid_val = params_fid_comet['As']
			# One-time Pell evaluation at fiducial cosmology to extract sigma12_fid
			params_fid_eval = dict(params_fid_comet)
			params_fid_eval.update({
				'b1': 2.0, 'b2': 0.0, 'c0': 0.0, 'c2': 0.0, 'c4': 0.0,
				'cnlo': 0.0, 'NP0': 0.0, 'NP20': 0.0, 'NP22': 0.0
			})
			if bias_basis == 'EggScoSmi':
				params_fid_eval.update({'g2': 0.0, 'g21': 0.0})
			elif bias_basis == 'AssBauGre':
				params_fid_eval.update({'bG2': 0.0, 'bGam3': 0.0})
			if 'vdg' in RSD_model:
				params_fid_eval['avir'] = 5.0
			if (RSD_model == 'pk_vdg') and ('Mnu' not in params_fid_eval):
				params_fid_eval['Mnu'] = 0.06
			k_fid = np.linspace(0.01, 0.3, 10)
			if (RSD_model == 'pk_vdg') or (RSD_model == 'pk_vdg_nonu'):
				_ = emu.Pell(k_fid, params_fid_eval, ell=[0], de_model=de_model)
			else:
				_ = emu.Xiell(np.linspace(20, 200, 10), params_fid_eval, ell=[0], de_model=de_model)
			sigma12_fid_val = float(np.squeeze(emu.params['s12']))
			data_dict[redshift[i]]['D_fid'] = D_fid_val
			data_dict[redshift[i]]['As_fid'] = As_fid_val
			data_dict[redshift[i]]['sigma12_fid'] = sigma12_fid_val
			print(f'Reparameterization: D_fid(z={redshift[i]}) = {D_fid_val:.6f}')
			print(f'Reparameterization: As_fid = {As_fid_val:.6f}')
			print(f'Reparameterization: sigma12_fid = {sigma12_fid_val:.6f}')
			print()
	...
	return data_dict, g21CoEvol, g2ExSet, bG2CoEvol, bGam3CoEvol, de_model, RSD_model, bias_basis, trueAs, use_AP_reparam, scale_avir_by_f

def execute(block, config):

	data_dict, g21CoEvol, g2ExSet, bG2CoEvol, bGam3CoEvol, de_model, RSD_model, bias_basis, trueAs, use_AP_reparam, scale_avir_by_f = config

	if RSD_model == "pk_vdg_nonu":
		setmnu = False
	else:
		setmnu = True

	params = load_cosmology(block, de_model, len(data_dict), mnu = setmnu, trueAs=trueAs)
	...
	if bias_basis == 'AssBauGre':
		nuisance_params = nuisance_params_AssBauGre
	elif bias_basis == 'EggScoSmi':
		nuisance_params = nuisance_params_EggScoSmi
	...
	for bias_bins in bias_spec:
		...
		if bias == 'bg2': bias = 'bG2' #reading in does not preserve capitalisation
		elif bias == 'bgam3': bias = 'bGam3'
		if bias in ["c0", "c2", "c4"]: 
		...
		elif bias == "avir" and RSD_model in ['EFT_vdg_EH', 'EFT_vdg']:
			params[bias].append(block.get_double("bias_spec", bias_bins))
		else: 
			params[bias].append(block.get_double("bias_spec", bias_bins))

	# AP + sigma12 reparameterization: convert tilded -> actual bias params
	if use_AP_reparam:
		for ii_r, data_set_r in enumerate(data_dict):
			cosmo_keys = ['wc', 'wb', 'h', 'As', 'ns']
			if de_model != 'lambda': cosmo_keys.append('w0')
			if de_model == 'w0wa': cosmo_keys.append('wa')
			if 'Mnu' in params: cosmo_keys.append('Mnu')
			params_cosmo_i = {key: float(params[key][ii_r]) for key in cosmo_keys if key in params}
			params_cosmo_i['z'] = data_set_r

			APfac, sigma12 = compute_reparam_factors(
				data_dict[data_set_r]['emu'], params_cosmo_i,
				data_dict[data_set_r]['sigma12_fid'],
				data_dict[data_set_r]['D_fid'],
				data_dict[data_set_r]['As_fid'],
				de_model)
			#print (f"Data set {data_set_r}: APfac = {APfac:.6f}, sigma12 = {sigma12:.6f}")
			...
			#print (f"Data set {data_set_r}: Rescaled b1 = {params['b1'][ii_r]:.6f}, b2 = {params['b2'][ii_r]:.6f}, c0 = {params['c0'][ii_r]:.6f}, c2 = {params['c2'][ii_r]:.6f}")
	...
	if bG2CoEvol:	
		params["bG2"] = [bG2f(b1) for b1 in params["b1"]]
	if bGam3CoEvol:
		params["bGam3"] = [bGam3f(b1, bG2) for b1, bG2 in zip(params["b1"], params["bG2"])]
	...
	for ii, data_set in enumerate(data_dict):
		...
		# If sampling f*avir, recover physical avir by dividing by growth rate f
		if scale_avir_by_f and 'avir' in params_i:
			emu_i = data_dict[data_set]['emu']
			Mnu_val = params_i.get('Mnu', 0.0)
			wnu = Mnu_val / emu_i.neutrino_mass_fac
			Om0 = (params_i['wc'] + params_i['wb'] + wnu) / params_i['h']**2
			H0 = 100.0 * params_i['h']
			w0_val = params_i.get('w0', -1.0)
			wa_val = params_i.get('wa', 0.0)
			emu_i.cosmo.update_cosmology(Om0, H0, de_model=de_model, w0=w0_val, wa=wa_val)
			_, f_val = emu_i.cosmo.growth_factor(
				np.atleast_1d(params_i['z']), get_growth_rate=True)
			params_i['avir'] = params_i['avir'] / float(np.squeeze(f_val))
	...
	print ("Derived sigma12 = "+str(block['cosmological_parameters', 'sigma12']))
	return 0
